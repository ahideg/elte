...
class Reduction { 
  ...
  Optional<Expression> reduce(final Expression exp) {    
    ...
    while(!finito) {
      if(result.isPresent() && isVal(result.get())) {
        finito = true;  
      } else {      
  ...
  private Optional<Expression> reduce_7_10(final Expression ex) {
    ...
    if(ex instanceof Rec) {
      final Rec rec = (Rec)ex;  //System.out.println(45645645); System.out.println(rec.getRightExpression()); 
  ...
  private Optional<Expression> reduce_7_12(final Expression ex) {
    ...
    if(ex instanceof Rec) {
      ...
      if(se instanceof Suc && isVal(se)) {
        ...
        substituteVar(rec.getFun().getExpression(), rec.getFun().getVar().getName(), e);
        result = Optional.of(rec.getFun().getExpression());
      }
    }
    return result;
  } 
  ...
  private Optional<Expression> reduce_7_13(final Expression ex) {
    ...
    if(ex instanceof App) {
      ...
      final Optional<Expression> ev = reduce(app.getLeftExpression());   // System.out.println(10124);       
      if(ev.isPresent()) {
    ...
    if(ex instanceof App) {
      ...
      //if(isVal(e1)) {
        ...
        if(el instanceof Lam) {
          ...
          substituteVar(lam.getExpression(), lam.getVar().getName(), e1);
          result = Optional.of(lam.getExpression()); 
        }
      //}
    }
    return result;
